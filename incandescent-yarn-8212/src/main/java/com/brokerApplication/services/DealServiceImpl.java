...
public class DealServiceImpl implements DealService{
	...
	public Deal addDealOfferFromCustomer(CustomerOffer customerOffer) {
		...
		if(customerOffer.getDealType()==DealType.RENT) {
		        ...
		        if(customerOffer.getStartPeriod().isBefore(LocalDateTime.now())) throw new DealException("The start period for renting the property is in the past, but it must be in the future.");
		...
		if(deal.getDealType()==DealType.RENT) {
			deal.setStartPeriod(customerOffer.getStartPeriod());
			deal.setEndPeriod(customerOffer.getEndPeriod());
		}
	...
	public Deal editDealOfferFromCustomerByDealId(Integer dealId, CustomerOffer customerOffer) {
		...
		if(deal.getDealType()==DealType.RENT) {
			...
			deal.setStartPeriod(null);
			deal.setEndPeriod(null);
		}
